...
workflow PROCESS_MONITORING {
    ...
    main:
        ...
        monitored = input_channel
            .map { items ->
                // Log process start with detailed channel state (only when debug_verbose enabled)
            ...
            .filter { items ->
                ...
                if (params.debug_verbose) { log.info "Validation passed for ${process_name}" }
                return true
            }
            .map { items ->
                // Final logging before passing items downstream (only when debug_verbose enabled)
                if (params.debug_verbose) {
                    if (items.size() > 0 && items[0] instanceof Map) {
