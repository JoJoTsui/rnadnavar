...
def main():
    ...
    dna_caller_variants = (
        {} if args.consensus_only else read_caller_vcfs(args.dna_vcf, "DNA")
    )
    ...
    rna_caller_variants = (
        {} if args.consensus_only else read_caller_vcfs(args.rna_vcf, "RNA")
    )
    ...
    print(
        f"    - DNA sources: {1 + len(dna_caller_variants)} (1 consensus + {len(dna_caller_variants)} callers)"
    )
    print(
    ...
    rescued_count = sum(
        1 for data in variant_data.values() if data.get("rescued", False)
    )
    cross_modality_count = sum(
        1 for data in variant_data.values() if data.get("cross_modality", False)
    )
    ...
    all_caller_names = [
        name for name, _, _ in all_collections if not name.endswith("_consensus")
    ]
