...
# Add bin/common to path for imports (sibling directory)
bin_common_path = Path(__file__).parent.parent / "common"
...
class TieringEngine:
    ...
    def compute_caller_tier(self, dna_caller_count: int, rna_caller_count: int) -> str:
        """
    ...
    def compute_final_tier(self, caller_tier: str, database_tier: str) -> str:
        """
    ...
    def compute_tier(
        ...
        dna_count, rna_count = count_concordant_callers(
            final_filter, filter_normalized_fields
        )
...
def compute_tiers_for_vcf(vcf_path: str, limit: Optional[int] = None) -> list:
    ...
    try:
        ...
        raise ImportError(
            "cyvcf2 is required for VCF processing. Install with: pip install cyvcf2"
        )
...
def print_tier_summary(tier_info: Dict[str, Any]) -> None:
    ...
    print("\nCaller Support:")
    print(
        f"  DNA Callers: {tier_info['dna_caller_count']} {tier_info['concordant_callers']['DNA']}"
    )
    print(
        f"  RNA Callers: {tier_info['rna_caller_count']} {tier_info['concordant_callers']['RNA']}"
    )
    print("\nDatabase Support:")
...
if __name__ == "__main__":
    ...
    tier = engine.compute_tier_simple(
        dna_caller_count=2, rna_caller_count=1, has_database_support=False
    )
