...
# Import chromosome utilities for canonical chromosome filtering
...
def argparser():
    parser = argparse.ArgumentParser(
        description="Filter VCF files with RaVeX filtering logic"
    )
    parser.add_argument("-i", "--input", help="VCF file input", required=True)
    parser.add_argument(
        "-o", "--output", help="VCF file output", default="filtered.vcf.gz"
    )
    parser.add_argument(
        "-g",
        "--gnomad_thr",
        help=f"Gnomad threshold for variants (default: {DEFAULT_THRESHOLDS['filtering_gnomad_threshold']})",
        default=DEFAULT_THRESHOLDS["filtering_gnomad_threshold"],
        type=float,
    )
    parser.add_argument(
        "--whitelist", help="BED file with variants to keep (CHROM POS REF ALT)"
    )
    parser.add_argument(
        "--blacklist", help="BED file with regions to remove (CHROM START END)"
    )
    parser.add_argument(
        "--filters",
        help="Other filters to be considered as PASS",
        default=["PASS"],
        nargs="+",
    )
    ...
    parser.add_argument(
        "--min_alt_reads",
        help=f"Minimum alt reads (default: {DEFAULT_THRESHOLDS['filtering_min_alt_reads']})",
        default=DEFAULT_THRESHOLDS["filtering_min_alt_reads"],
        type=int,
    )
    parser.add_argument(
        "--filter_multiallelic",
        help="Filter out multiallelic sites (multiple ALT alleles)",
        action="store_true",
    )
    parser.add_argument(
        "--output_stripped",
        help="Output stripped VCF file (multiallelic-filtered, FORMAT-stripped). If not provided, will be <output>.stripped.vcf.gz",
    )
    parser.add_argument(
...
def filter_homopolymer(ref_context, alt, hp_length=6):
    ...
    context_to_consider = ref_context[
        length_to_consider - hp_length + 1 : length_to_consider + hp_length
    ]
...
def get_csq_field_cyvcf2(variant, field_name):
    ...
    csq_fields = {"Consequence": 1, "BIOTYPE": 7, "SYMBOL": 3}
...
def apply_filters(vcf_in, args, genome, include_non_canonical=False):
    ...
    for variant in vcf_in:
        # Chromosome filtering - skip non-canonical chromosomes unless explicitly included
...
def write_filtered_vcf(
    filtered_variants, input_vcf_path, output_path, strip_format=False
):
    ...
    if "RaVeX_FILTER" not in new_header.filters:
        new_header.filters.add("RaVeX_FILTER", None, None, "RaVeX filtering applied")
...
def main():
    ...
    if not args.output_stripped:
        ...
        args.output_stripped = str(
            output_path.parent / f"{output_path.stem}.stripped.vcf.gz"
        )
    ...
    write_vcf_stripped(
        filtered_variants_stripped, args.input, args.output_stripped, use_cyvcf2=True
    )
    ...
    for vcf_path in [args.output, args.output_stripped]:
        try:
            subprocess.run(
                ["tabix", "-p", "vcf", vcf_path], check=True, capture_output=True
            )
