...
def pileup(samtools,bam,ref,site):
    c,pos,r,a=site
    cmd=[samtools,'mpileup','-f',ref,'-r',f'{c}:{pos}-{pos}','-q','0','-Q','0',bam]
...
def main():
    ...
    ap.add_argument('--out',required=True); ap.add_argument('--min-tumor-alt',type=int,default=3); ap.add_argument('--max-normal-alt',type=int,default=0); ap.add_argument('--min-dp',type=int,default=10); ap.add_argument('--samtools',default='samtools')
    ...
    for site in parse_sites(args.candidates):
        ...
        try:
            td,tb=pileup(sam,args.tumor_bam,args.reference,site); nd,nb=pileup(sam,args.normal_bam,args.reference,site)
        ...
        out.append(row)
    Path(args.out).write_text(json.dumps({'rules':{'min_tumor_alt':args.min_tumor_alt,'max_normal_alt':args.max_normal_alt,'min_dp':args.min_dp},'results':out},indent=2)+'\n')
