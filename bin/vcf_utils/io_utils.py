...
def create_output_header(
    ...
    Args:
        ...
        include_non_canonical (bool, optional): If True, include all chromosomes
            in header. If False (default), only include canonical chromosomes.
    ...
    # Add lines from template (excluding sample line and filtering contigs if needed)
    for line in header_str.strip().split("\n"):
        if line.startswith("#CHROM"):
            continue
        if line.startswith("#"):
            # Filter contig lines if not including non-canonical
            if line.startswith("##contig=<ID=") and not include_non_canonical:
                # Extract chromosome name from contig line
                chrom_name = line.split("ID=")[1].split(",")[0].split(">")[0]
                if not is_canonical_chromosome(chrom_name):
                    continue  # Skip non-canonical contigs
    ...
    add_info_safe(
        ...
        "1",
        "Integer",
        "Total number of aggregated variant callers (excludes consensus)",
    )
    add_info_safe(
        new_header,
        "CALLERS",
        ".",
        "String",
        "List of all aggregated variant callers with modality prefix (pipe-separated)",
    )
    add_info_safe(
        new_header,
        "N_SUPPORT_CALLERS",
        "1",
        "Integer",
        "Number of variant callers that detected this variant (excludes consensus)",
    )
    add_info_safe(
        new_header,
        "CALLERS_SUPPORT",
        ".",
        "String",
        "Variant callers that detected this variant with modality prefix (pipe-separated, excludes consensus)",
    )
    add_info_safe(
        new_header,
        "N_CONSENSUS_SUPPORT",
        "1",
        "Integer",
        "Number of consensus callers that detected this variant",
    )
    add_info_safe(
        new_header,
        "CONSENSUS_SUPPORT",
        ".",
        "String",
        "Consensus callers that detected this variant with modality prefix (pipe-separated)",
    )
    add_info_safe(
        new_header,
        "N_DNA_CALLERS",
        "1",
        "Integer",
        "Total number of DNA variant callers used in analysis",
    )
    add_info_safe(
        new_header,
        "N_RNA_CALLERS",
        "1",
        "Integer",
        "Total number of RNA variant callers used in analysis",
    )
    add_info_safe(
        new_header,
        ...
        "1",
        "Integer",
        "Number of DNA callers that detected this variant",
    )
    add_info_safe(
        new_header,
    ...
    add_info_safe(
        ...
        ".",
        "String",
        "Original filter values from each caller with modality prefix (format: MODALITY_caller:filter|..., excludes consensus)",
    )
    add_info_safe(
        new_header,
        "FILTERS_NORMALIZED",
        ".",
        "String",
        "Normalized filter categories with modality prefix (format: MODALITY_caller:filter|..., excludes consensus)",
    )
    add_info_safe(
        new_header,
        "FILTERS_CATEGORY",
        ".",
        "String",
        "Filter categories with modality prefix (format: MODALITY_caller:category|..., excludes consensus)",
    )
    add_info_safe(
        new_header,
        "UNIFIED_FILTER",
        "1",
        "String",
        "Unified filter status based on majority of all callers",
    )
    add_info_safe(
        new_header,
        "UNIFIED_FILTER_DNA",
        "1",
        "String",
        "Unified filter status for DNA callers",
    )
    add_info_safe(
        new_header,
        "UNIFIED_FILTER_RNA",
        "1",
        "String",
        "Unified filter status for RNA callers",
    )
    ...
    add_info_safe(
        ...
        "1",
        "String",
        "Whether variant passes consensus threshold overall (YES/NO)",
    )
    add_info_safe(
        new_header,
    ...
    # Quality aggregation
    add_info_safe(
        new_header, "QUAL_MEAN", "1", "Float", "Mean QUAL score across callers"
    )
    add_info_safe(
        new_header, "QUAL_MIN", "1", "Float", "Minimum QUAL score across callers"
    )
    add_info_safe(
        new_header, "QUAL_MAX", "1", "Float", "Maximum QUAL score across callers"
    )

    # Genotype aggregation
    add_info_safe(
        new_header, "CONSENSUS_GT", "1", "String", "Consensus genotype across callers"
    )
    add_info_safe(
        new_header,
        "GT_BY_CALLER",
        ".",
        "String",
        "Genotypes from each caller with modality prefix (format: MODALITY_caller:GT|...)",
    )
    ...
    add_info_safe(
        ...
        "1",
        "Float",
        "Mean variant allele frequency across callers",
    )
    ...
    if include_rescue_fields:
        add_info_safe(
            ...
            ".",
            "String",
            "Modalities where variant was detected (pipe-separated)",
        )
        add_info_safe(
            new_header,
            "CALLERS_BY_MODALITY",
            ".",
            "String",
            "Callers grouped by modality (format: modality:caller1,caller2|...)",
        )
        add_info_safe(
            new_header,
            "DNA_SUPPORT",
            "1",
            "Integer",
            "Number of DNA callers supporting this variant",
        )
        add_info_safe(
            new_header,
            "RNA_SUPPORT",
            "1",
            "Integer",
            "Number of RNA callers supporting this variant",
        )
        add_info_safe(
            new_header,
            "CROSS_MODALITY",
        ...
        add_info_safe(
            new_header, "DP_DNA_MEAN", "1", "Float", "Mean depth across DNA callers"
        )
        add_info_safe(
            new_header, "DP_RNA_MEAN", "1", "Float", "Mean depth across RNA callers"
        )
        add_info_safe(
            new_header, "VAF_DNA_MEAN", "1", "Float", "Mean VAF across DNA callers"
        )
        add_info_safe(
            new_header, "VAF_RNA_MEAN", "1", "Float", "Mean VAF across RNA callers"
        )
    ...
    add_info_safe(
        ...
        "1",
        "String",
        "Variant Classification: Somatic, Germline, Reference, or Artifact",
    )
    add_info_safe(
        new_header,
        "VC_CALLERS",
        ".",
        "String",
        "Classification by each caller (format: caller:classification|...)",
    )
    add_info_safe(
        new_header,
        "VC_CONSENSUS",
    ...
    add_filter_safe(
        ...
        None,
        None,
        "High-confidence somatic variant specific to tumor",
    )
    add_filter_safe(
        new_header, "Germline", None, None, "Germline variant detected in normal sample"
    )
    add_filter_safe(
        new_header, "Reference", None, None, "Reference call - no variant detected"
    )
    add_filter_safe(
        new_header,
        "Artifact",
...
def write_union_vcf(
    ...
    for vkey, data in sorted_variants:
        ...
        actual_callers_in_variant = [
            c for c in data["callers"] if not is_consensus_caller(c)
        ]
        consensus_callers_in_variant = [
            c for c in data["callers"] if is_consensus_caller(c)
        ]
        ...
        if modality_map:
            ...
            total_dna_callers = sum(
                1 for c in all_callers if modality_map.get(c) == "DNA"
            )
            total_rna_callers = sum(
                1 for c in all_callers if modality_map.get(c) == "RNA"
            )
            ...
            dna_callers_support = sum(
                1 for c in actual_callers_in_variant if modality_map.get(c) == "DNA"
            )
            rna_callers_support = sum(
                1 for c in actual_callers_in_variant if modality_map.get(c) == "RNA"
            )
        ...
        for i, caller in enumerate(data["callers"]):
            if not is_consensus_caller(caller):  # Skip consensus callers
                ...
                if i < len(data["filters_normalized"]):
                    ...
                    prefixed_filters_normalized.append(
                        f"{prefixed_caller}:{filter_val}"
                    )
        ...
        record.info["FILTERS_ORIGINAL"] = (
            "|".join(prefixed_filters_original) if prefixed_filters_original else "."
        )
        record.info["FILTERS_NORMALIZED"] = (
            "|".join(prefixed_filters_normalized)
            if prefixed_filters_normalized
            else "."
        )
        record.info["FILTERS_CATEGORY"] = (
            "|".join(prefixed_filters_category) if prefixed_filters_category else "."
        )
        ...
        if modality_map:
            ...
            has_dna_callers = any(
                not is_consensus_caller(c) and modality_map.get(c) == "DNA"
                for c in data["callers"]
            )
            has_rna_callers = any(
                not is_consensus_caller(c) and modality_map.get(c) == "RNA"
                for c in data["callers"]
            )
        ...
        if modality_map:
            ...
            record.info["MODALITIES"] = (
                "|".join(sorted(modalities)) if modalities else "."
            )
            ...
            for i, caller in enumerate(data["callers"]):
                if modality_map.get(caller) == "DNA":
                    if (
                        i < len(agg["dp_by_caller"])
                        and agg["dp_by_caller"][i] is not None
                    ):
                        dna_dp_values.append(agg["dp_by_caller"][i])
                    if (
                        i < len(agg["vaf_by_caller"])
                        and agg["vaf_by_caller"][i] is not None
                    ):
            ...
            for i, caller in enumerate(data["callers"]):
                if not is_consensus_caller(caller):  # Skip consensus callers
                    ...
                    if i < len(agg["gt_by_caller"]):
                        prefixed_gt_by_caller.append(
                            f"{prefixed_caller}:{agg['gt_by_caller'][i]}"
                        )
            ...
            for i, caller in enumerate(data["callers"]):
                if not is_consensus_caller(caller):  # Skip consensus callers
                    ...
                    if i < len(agg["dp_by_caller"]):
                        dp_val = (
                            ""
                            if agg["dp_by_caller"][i] is None
                            else str(agg["dp_by_caller"][i])
                        )
            ...
            for i, caller in enumerate(data["callers"]):
                if not is_consensus_caller(caller):  # Skip consensus callers
                    ...
                    if i < len(agg["vaf_by_caller"]):
                        vaf_val = (
                            ""
                            if agg["vaf_by_caller"][i] is None
                            else f"{agg['vaf_by_caller'][i]:.4f}"
                        )
