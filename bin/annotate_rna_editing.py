...
NOTE: Uses unified configuration from classification_config.py for default thresholds.
...
class StructuredLogger:
    ...
    def log_performance(
        self, operation: str, duration: float, items_processed: int = 0
    ):
        """Log performance metrics."""
        rate = items_processed / duration if duration > 0 and items_processed > 0 else 0
        self.logger.info(
            f"PERFORMANCE: {operation} - duration: {duration:.2f}s, items: {items_processed}, rate: {rate:.0f}/s"
        )
    ...
    def log_resource_usage(self, stage: str):
        ...
        try:
            ...
            self.logger.info(
                f"RESOURCES: {stage} - memory: {memory_mb:.1f}MB, cpu: {cpu_percent:.1f}%"
            )
        except ImportError:
    ...
    def get_metrics_summary(self) -> Dict:
        ...
        return {"metrics": self.metrics, "total_elapsed": time.time() - self.start_time}
...
class RNAEditingAnnotator:
    ...
    def __init__(
        ...
        self.operational_logger = get_operational_logger(
            name="rna_editing_annotation", output_dir=self.output_vcf.parent
        )
        ...
        logger.info(
            "=== RNA Editing Annotation Pipeline Started (Enhanced with Operational Monitoring) ==="
        )
        ...
        self.operational_logger.log_metric(
            "min_rna_support", self.min_rna_support, "gauge"
        )
        ...
        if missing_tools:
            ...
            logger.error(
                "Please ensure all required tools are installed and available in PATH"
            )
        ...
        try:
            ...
            if not self.error_handler.handle_error(
                e, "initialization", fallback_action
            ):
    ...
    def validate_inputs(self) -> None:
        ...
        try:
            ...
            structured_logger.log_metric(
                "input_vcf_size_mb", input_size / 1024 / 1024, "MB"
            )
            structured_logger.log_metric("rediportal_vcf_size_bytes", rediportal_size)
            structured_logger.log_metric(
                "rediportal_vcf_size_mb", rediportal_size / 1024 / 1024, "MB"
            )
            structured_logger.log_metric(
                "total_input_size_mb", total_input_size / 1024 / 1024, "MB"
            )

            logger.info(
                f"Input VCF size: {input_size:,} bytes ({input_size / 1024 / 1024:.1f} MB)"
            )
            logger.info(
                f"REDIportal VCF size: {rediportal_size:,} bytes ({rediportal_size / 1024 / 1024:.1f} MB)"
            )
            logger.info(
                f"Total input size: {total_input_size:,} bytes ({total_input_size / 1024 / 1024:.1f} MB)"
            )

            # Estimate resource requirements based on file sizes
            estimated_memory_mb = max(
                500, (total_input_size / 1024 / 1024) * 2
            )  # 2x input size
            estimated_disk_gb = max(
                2, (total_input_size / 1024 / 1024 / 1024) * 3
            )  # 3x input size

            # Log resource estimates for monitoring
            structured_logger.log_metric(
                "estimated_memory_mb", estimated_memory_mb, "MB"
            )
            structured_logger.log_metric("estimated_disk_gb", estimated_disk_gb, "GB")
            structured_logger.log_metric(
                "available_disk_gb", resource_stats["disk_free_gb"], "GB"
            )
    ...
    def run_annotation(self) -> None:
        ...
        logger.info(
            "Starting integrated RNA editing annotation pipeline with comprehensive monitoring..."
        )
        logger.info(
            "Pipeline components: REDIportal converter + bcftools VCF-to-VCF + evidence tiering + FILTER updates"
        )
        logger.info(
            "Monitoring: Performance metrics, resource usage, processing statistics"
        )
        ...
        try:
            ...
            logger.info(
                "Step 2: Running bcftools VCF-to-VCF annotation with exact matching..."
            )
            annotated_vcf = self._run_bcftools_annotation_with_engine(
                prepared_rediportal
            )
            ...
            structured_logger.log_performance(
                "complete_pipeline", pipeline_time, self.stats.get("total_variants", 0)
            )
            structured_logger.log_metric(
                "pipeline_duration_seconds", pipeline_time, "s"
            )
            structured_logger.log_metric(
                "pipeline_duration_minutes", pipeline_time / 60, "min"
            )
            structured_logger.log_resource_usage("pipeline_complete")
            structured_logger.log_stage("ANNOTATION_PIPELINE", "SUCCESS")

            logger.info(
                "✓ Integrated RNA editing annotation pipeline completed successfully!"
            )
            logger.info(
                "All components integrated: REDIportal conversion, bcftools annotation, evidence tiering, FILTER updates"
            )
            ...
            if self.output_vcf.exists():
                try:
                    ...
                    logger.warning(
                        f"Failed to clean up partial output: {cleanup_error}"
                    )
    ...
    def _prepare_rediportal_database_with_converter(self) -> str:
        ...
        try:
            ...
            if not prepared_file.endswith(".vcf.gz") and not prepared_file.endswith(
                ".vcf"
            ):
                logger.warning(
                    f"Prepared file may not be in VCF format: {prepared_file}"
                )
                self.stats["warnings"].append(
                    "REDIportal converter output format unclear"
                )
            ...
            self.stats["processing_steps"].append(
                ("prepare_rediportal_converter", step_time)
            )

            logger.info(
                f"✓ REDIportal database prepared as VCF: {prepared_file} ({step_time:.2f}s)"
            )
    ...
    def _run_bcftools_annotation_with_engine(self, rediportal_vcf: str) -> str:
        ...
        try:
            ...
            temp_annotated = (
                self.output_vcf.parent / f"temp_annotated_{int(time.time())}.vcf"
            )
            ...
            logger.info(
                "Executing VCF-to-VCF annotation with exact coordinate and allele matching..."
            )
            ...
            if hasattr(annotator, "stats"):
                for step_name, step_time in annotator.stats.get("processing_steps", []):
                    self.stats["processing_steps"].append(
                        (f"annotator_{step_name}", step_time)
                    )
            ...
            if hasattr(annotator, "operation_logger"):
                ...
                logger.info(
                    f"bcftools operations: {operation_stats['successful_commands']} successful, "
                    f"{operation_stats['failed_commands']} failed"
                )

            step_time = time.time() - step_start
            self.stats["processing_steps"].append(
                ("bcftools_annotation_engine", step_time)
            )

            logger.info(
                f"✓ bcftools VCF-to-VCF annotation completed: {temp_annotated} ({step_time:.2f}s)"
            )
    ...
    def _process_variants_with_pysam_and_evidence_tiering(
        self, annotated_vcf: str
    ) -> None:
        """
        ...
        step_start = time.time()
        logger.info(
            "Processing variants with pysam, evidence tiering, FILTER updates, and performance optimization..."
        )
        ...
        try:
            ...
            memory_monitor = MemoryMonitor(
                max_memory_mb=6000
            )  # 6GB limit for variant processing
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "pysam not available - skipping evidence tiering and FILTER updates"
                )
            ...
            logger.info(
                f"Initializing evidence tiering processor (min_rna_support={self.min_rna_support})..."
            )
            evidence_processor = create_evidence_tiering_processor(
            ...
            output_vcf = pysam.VariantFile(
                str(self.output_vcf), "w", header=output_header
            )
            ...
            try:
                logger.info(
                    "Processing variants for evidence tiering and FILTER updates with performance optimization..."
                )
                ...
                for variant in input_vcf:
                    ...
                    canonical = is_canonical_editing_transition(
                    ...
                    tiering_result = evidence_processor.process_variant(
                        variant_data, rediportal_match
                    )
                    ...
                    evidence_stats["evidence_distribution"][
                        tiering_result["evidence_tier"]
                    ] += 1

                    # Get original FILTER value for preservation logic
                    original_filter = (
                        list(variant.filter.keys())[0]
                        if variant.filter.keys()
                        else "PASS"
                    )

                    # Update FILTER using dedicated FILTER updater with complete criteria
                    new_filter, filter_was_updated = (
                        filter_updater.update_variant_filter(
                    ...
                    for key, value in variant.info.items():
                        try:
                            ...
                            logger.debug(
                                f"Skipping INFO field {key} due to pysam error: {e}"
                            )
                            continue

                    # Add RNA editing specific fields
                    output_variant.info["REDI_EVIDENCE"] = tiering_result[
                        "evidence_tier"
                    ]
                    ...
                    if len(variant.samples) > 0:
                        ...
                        if sample_name in output_header.samples:
                            for fmt_key in variant.format.keys():
                                try:
                                    output_variant.samples[sample_name][fmt_key] = (
                                        variant.samples[sample_name][fmt_key]
                                    )
                                except Exception as e:
                                    logger.debug(
                                        f"Skipping FORMAT field {fmt_key} due to pysam error: {e}"
                                    )
                                    continue
                    ...
                    if evidence_stats["total_variants"] % 10000 == 0:
                        ...
                        rate = (
                            evidence_stats["total_variants"] / elapsed_time
                            if elapsed_time > 0
                            else 0
                        )
                        logger.info(
                            f"Processed {evidence_stats['total_variants']:,} variants ({rate:.0f} variants/sec)..."
                        )

                        # Memory monitoring and optimization
                        if memory_monitor:
                            memory_stats = memory_monitor.check_memory(
                                f"processed {evidence_stats['total_variants']:,} variants"
                            )
                            ...
                            if evidence_stats["total_variants"] % 50000 == 0:
                                freed_mb = memory_monitor.force_garbage_collection(
                                    "periodic cleanup during variant processing"
                                )
                                if freed_mb > 5:  # Log if significant memory was freed
                                    logger.info(
                                        f"Freed {freed_mb:.1f} MB during variant processing"
                                    )
            ...
            step_time = time.time() - step_start
            self.stats["processing_steps"].append(
                ("pysam_evidence_processing", step_time)
            )

            # Log comprehensive processing results
            logger.info("✓ Evidence processing and FILTER updates completed:")
            logger.info(
                f"  Total variants processed: {evidence_stats['total_variants']:,}"
            )
            logger.info(
                f"  REDIportal matches: {evidence_stats['rediportal_matches']:,}"
            )
            logger.info(
                f"  Canonical transitions: {evidence_stats['canonical_transitions']:,}"
            )
            logger.info(
                f"  RNA consensus variants (>={self.min_rna_support} callers): {evidence_stats['rna_consensus_variants']:,}"
            )
            logger.info(
                f"  RNA-only variants (DNA=0): {evidence_stats['rna_only_variants']:,}"
            )
            logger.info(
                f"  FILTER updates to RNAedit: {evidence_stats['filter_updates']:,}"
            )
            logger.info(
                f"  FILTER values preserved: {evidence_stats['filter_preserved']:,}"
            )
            ...
            for level, count in evidence_stats["evidence_distribution"].items():
                percentage = (
                    (count / evidence_stats["total_variants"] * 100)
                    if evidence_stats["total_variants"] > 0
                    else 0
                )
    ...
    def _validate_and_index_output(self) -> None:
        ...
        try:
            ...
            if not self.output_vcf.exists():
                ...
                logger.error(
                    f"Output directory exists: {self.output_vcf.parent.exists()}"
                )
                logger.error(
                    f"Output directory writable: {os.access(self.output_vcf.parent, os.W_OK)}"
                )
                raise RuntimeError(error_msg)
            ...
            logger.info(
                f"✓ Output file validated: {file_size:,} bytes ({file_size / 1024 / 1024:.1f} MB)"
            )
            ...
            step_time = time.time() - step_start
            self.stats["processing_steps"].append(
                ("validate_and_index_output", step_time)
            )

            logger.info(
                f"✓ Output validation and indexing completed ({step_time:.2f}s)"
            )
    ...
    def _validate_vcf_format(self) -> None:
        ...
        try:
            ...
            if not first_line.startswith("##fileformat=VCF"):
                logger.warning(
                    f"Output file may not have valid VCF header. First line: {first_line}"
                )
    ...
    def _cleanup_temp_files(
        self, temp_files: list, force_cleanup: bool = False
    ) -> None:
        """
            ...
            if temp_file != self.input_vcf and temp_file != self.rediportal_vcf:
                try:
                    if temp_file.exists():
                        ...
                        temp_file.unlink()
                        logger.debug(
                            f"Cleaned up temporary file: {temp_file} ({file_size:,} bytes)"
                        )
                        cleaned_count += 1
                    ...
                    for suffix in [".tbi", ".csi"]:
                        ...
                        if index_file.exists():
                            ...
                            index_file.unlink()
                            logger.debug(
                                f"Cleaned up index file: {index_file} ({index_size:,} bytes)"
                            )
                    ...
                    if force_cleanup:
                        try:
                            if temp_file.exists():
                                ...
                                temp_file.unlink()
                                logger.info(
                                    f"Force cleaned up temporary file: {temp_file}"
                                )
                                cleaned_count += 1
                        except Exception as force_error:
                            logger.error(
                                f"Force cleanup also failed for {temp_file}: {force_error}"
                            )

        if cleaned_count > 0:
            logger.info(
                f"✓ Cleaned up {cleaned_count} temporary files ({total_size_cleaned:,} bytes freed)"
            )
        if failed_count > 0:
    ...
    def _ensure_output_file_permissions(self) -> None:
        ...
        try:
            ...
            step_time = time.time() - step_start
            self.stats["processing_steps"].append(
                ("ensure_output_permissions", step_time)
            )
            logger.info(
                f"✓ Output file permissions set successfully ({step_time:.2f}s)"
            )
    ...
    def _create_output_index(self) -> None:
        ...
        try:
            ...
            if not index_file.exists():
                raise RuntimeError(
                    "Tabix indexing completed but index file was not created"
                )
            ...
            logger.info(
                f"✓ Tabix index created successfully ({index_size:,} bytes, {step_time:.2f}s)"
            )
    ...
    def _generate_summary_statistics(self) -> None:
        ...
        logger.info(
            "=== RNA Editing Annotation Summary (Enhanced with Operational Metrics) ==="
        )
        logger.info(
        ...
        if "total_variants" in self.stats and self.stats["total_variants"] > 0:
            processing_rate = (
                self.stats["total_variants"] / total_time if total_time > 0 else 0
            )
            structured_logger.log_metric(
                "final_processing_rate", processing_rate, "variants/s"
            )
        ...
        if "annotated_variants" in self.stats:
            annotation_rate = (
                (self.stats["annotated_variants"] / self.stats["total_variants"] * 100)
                if self.stats["total_variants"] > 0
                else 0
            )
            structured_logger.log_metric(
                "annotation_rate_percent", annotation_rate, "%"
            )
            logger.info(
                f"Annotation rate: {annotation_rate:.1f}% ({self.stats['annotated_variants']:,} variants annotated)"
            )
        ...
        try:
            if self.input_vcf.exists():
                input_size = self.input_vcf.stat().st_size
                logger.info(
                    f"Input file size: {input_size:,} bytes ({input_size / 1024 / 1024:.1f} MB)"
                )

            if self.rediportal_vcf.exists():
                rediportal_size = self.rediportal_vcf.stat().st_size
                logger.info(
                    f"REDIportal file size: {rediportal_size:,} bytes ({rediportal_size / 1024 / 1024:.1f} MB)"
                )

            if self.output_vcf.exists():
                output_size = self.output_vcf.stat().st_size
                logger.info(
                    f"Output file size: {output_size:,} bytes ({output_size / 1024 / 1024:.1f} MB)"
                )
        ...
        if self.stats["processing_steps"]:
            ...
            if overhead_time > 0:
                overhead_percentage = (
                    (overhead_time / total_time * 100) if total_time > 0 else 0
                )
                logger.info(
                    f"  overhead/other: {overhead_time:.2f}s ({overhead_percentage:.1f}%)"
                )
        ...
        if "output_line_count" in self.stats:
            ...
            logger.info(
                f"  Variant lines: {self.stats.get('output_variant_count', 0):,}"
            )
            logger.info(
                f"  Total lines checked: {self.stats.get('output_line_count', 0):,}"
            )
        ...
        if self.stats["errors"]:
            ...
            for error_info in self.stats["errors"]:
                ...
                logger.error(
                    f"  {error_info['context']}: {error_info['error_type']} - {error_info['error_message']}"
                )
        ...
        if "evidence_tiering_stats" in self.stats:
            ...
            logger.info(
                f"  Total variants processed: {stats['total_variants_processed']:,}"
            )
            logger.info(
                f"  RNA consensus variants: {stats['rna_consensus_variants']:,} ({stats['rna_consensus_rate']:.1f}%)"
            )
            logger.info(
                f"  RNA-only variants: {stats['rna_only_variants']:,} ({stats['rna_only_rate']:.1f}%)"
            )
            logger.info(
                f"  FILTER updates to RNAedit: {stats['filter_updates']:,} ({stats['filter_update_rate']:.1f}%)"
            )
            ...
            if "evidence_levels" in stats:
                ...
                for level, count in stats["evidence_levels"].items():
                    percentage = (
                        (count / stats["total_variants_processed"] * 100)
        ...
        if "filter_update_stats" in self.stats:
            ...
            logger.info(
                f"  Total variants processed: {filter_stats['total_variants_processed']:,}"
            )
            logger.info(
                f"  FILTER updates to RNAedit: {filter_stats['filter_updates_to_rnaedit']:,} ({filter_stats['filter_update_rate']:.1f}%)"
            )
            logger.info(
                f"  FILTER values preserved: {filter_stats['filter_values_preserved']:,} ({filter_stats['filter_preservation_rate']:.1f}%)"
            )

            # Log FILTER distribution changes
            if (
                "original_filter_distribution" in filter_stats
                and "final_filter_distribution" in filter_stats
            ):
                logger.info("  FILTER value changes:")
                all_filters = set(
                    filter_stats["original_filter_distribution"].keys()
                ) | set(filter_stats["final_filter_distribution"].keys())
                for filter_val in sorted(all_filters):
                    original_count = filter_stats["original_filter_distribution"].get(
                        filter_val, 0
                    )
                    final_count = filter_stats["final_filter_distribution"].get(
                        filter_val, 0
                    )
                    ...
                    if change != 0:
                        ...
                        logger.info(
                            f"    {filter_val}: {original_count:,} → {final_count:,} {change_str}"
                        )

        # Log performance metrics
        if (
            total_time > 0
            and "total_variants" in self.stats
            and self.stats["total_variants"] > 0
        ):
    ...
    def _save_structured_metrics(
        self, metrics_summary: Dict, total_time: float, success: bool
    ) -> None:
        ...
        try:
            ...
            metrics_document = {
                ...
                "performance_metrics": {
                    ...
                    "processing_rate_variants_per_second": self.stats.get(
                        "total_variants", 0
                    )
                    / total_time
                    if total_time > 0
                    else 0,
                    "memory_efficient": total_time
                    < 300,  # Under 5 minutes considered efficient
                    "large_file_processing": self.stats.get("total_variants", 0)
                    > 100000,
                },
                ...
                "resource_utilization": {
                    ...
                    "disk_space_sufficient": len(
                        [w for w in self.stats["warnings"] if "disk space" in w.lower()]
                    )
                    == 0,
                    "processing_steps": [
                        {
                            ...
                            "percentage_of_total": (step_time / total_time * 100)
                            if total_time > 0
                            else 0,
                        }
                        for step_name, step_time in self.stats["processing_steps"]
                ...
                "operational_status": {
                    ...
                    if success and len(self.stats["warnings"]) < 5
                    else "degraded"
                    if success
                    else "failed",
                    "requires_attention": len(self.stats["errors"]) > 0
                    or len(self.stats["warnings"]) > 10,
            ...
            metrics_file = (
                self.output_vcf.parent / f"rna_editing_metrics_{int(time.time())}.json"
            )
    ...
    def _generate_operational_recommendations(
        self, total_time: float, success: bool
    ) -> None:
        ...
        if total_time > 1800:  # Over 30 minutes
            logger.info(
                "PERFORMANCE: Consider increasing allocated resources for faster processing"
            )
        ...
        if self.stats["warnings"] or self.stats["errors"]:
            ...
            if self.stats["warnings"]:
                logger.info(
                    "- Review warning messages for potential data quality issues"
                )
...
Examples:
  ...
  python annotate_rna_editing.py -i input.vcf.gz -r /path/to/rediportal.vcf.gz -o output.vcf.gz --verbose
    ...
    parser.add_argument(
        "--min-rna-support",
        type=int,
        default=DEFAULT_THRESHOLDS["rna_editing_min_support"],
        metavar="N",
        help=f"Minimum RNA caller support threshold (default: {DEFAULT_THRESHOLDS['rna_editing_min_support']}, must be >= 1)",
    )
