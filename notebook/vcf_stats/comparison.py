...
class WorkflowComparator:
    ...
    def _extract_rna_stats(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stage, files in stats.items():
            ...
            for name, data in files.items():
                ...
                # RNA samples typically have "RNA" in the name
                if "RNA" in name.upper() or "rna" in name:
                    rna_files[name] = data
    ...
    def _extract_dna_stats(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for stage, files in stats.items():
            ...
            for name, data in files.items():
                ...
                # DNA samples typically have "DNA" in the name and not "RNA"
                if "DNA" in name.upper() and "RNA" not in name.upper():
                    dna_files[name] = data
    ...
    def compare_rna_category_distribution(self) -> pd.DataFrame:
        ...
        for stage in VCF_STAGE_ORDER:
            ...
            all_categories = set(standard_classification.keys()) | set(realignment_classification.keys())
    ...
    def calculate_realignment_impact(self) -> Dict[str, Any]:
        ...
        if standard_total > 0:
            realignment_improvement = ((realignment_total - standard_total) / standard_total) * 100
        else:
    ...
    def export_comparison_report(self, output_dir: Path):
        ...
        for category, delta in realignment_impact["category_changes"].items():
            impact_rows.append({
                "Metric": f"Category Change: {category}",
                "Value": delta
            })
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            rna_variant_counts.to_excel(writer, sheet_name="RNA_Variant_Counts", index=False)
            rna_category_dist.to_excel(writer, sheet_name="RNA_Category_Distribution", index=False)
            rna_annotation.to_excel(writer, sheet_name="RNA_Annotation_Stages", index=False)
            integrative_view.to_excel(writer, sheet_name="Integrative_View", index=False)
