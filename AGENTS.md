...
- `io_utils.py` — VCF reading/writing with custom INFO fields (writes `CLASSIFICATION_RATIONALE`, per-caller `GT/DP/VAF/ALT_COUNT_BY_CALLER`, `RESCUE_PROMOTED`, and descriptive `ENS_SUPPORT` using the explicit expected caller panel in consensus mode; output is 8-column VCF, no sample column)
